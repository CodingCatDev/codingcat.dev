 ...
 * Configurable values (region, serveUrl) come from Sanity with env var fallback.
 * Secrets (AWS credentials) remain as process.env.
 */
export async function getRemotionConfig(): Promise<RemotionLambdaConfig> {
  ...
  if (!region) missing.push("REMOTION_AWS_REGION / remotion_config.awsRegion");
  if (!serveUrl) missing.push("REMOTION_SERVE_URL / remotion_config.serveUrl");

  if (missing.length > 0) {
    throw new Error(
      `[REMOTION] Missing required configuration: ${missing.join(", ")}. ` +
        `Set these in Sanity config or as environment variables. ` +
...
/**
 * Get the Lambda function name from Sanity config, env, or default.
 */
