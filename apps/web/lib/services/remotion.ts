...
export async function getRemotionConfig(): Promise<RemotionLambdaConfig> {
  ...
  const serveUrl = await getConfigValue("remotion_config", "remotionServeUrl", undefined);
  ...
  if (!serveUrl) missing.push("remotion_config.remotionServeUrl");
...
async function getFunctionName(): Promise<string> {
  return (
    (await getConfigValue(
      ...
      "remotion-render-4-0-431-mem2048mb-disk2048mb-240sec",
    )) ?? "remotion-render-4-0-431-mem2048mb-disk2048mb-240sec"
  );
}
