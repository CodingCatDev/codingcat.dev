...
export async function getGCSConfig(): Promise<GCSConfig> {
  const bucket = await getConfigValue("gcs_config", "gcsBucketName", "codingcatdev-content-engine");
  const projectId = await getConfigValue("gcs_config", "gcsProjectId", "codingcatdev");
  const clientEmail = process.env.GCS_CLIENT_EMAIL;
  let privateKey = process.env.GCS_PRIVATE_KEY;

  if (!bucket || !projectId || !clientEmail || !privateKey) {
    const missing = [
      !bucket && "gcs_config.gcsBucketName",
      !projectId && "gcs_config.gcsProjectId",
