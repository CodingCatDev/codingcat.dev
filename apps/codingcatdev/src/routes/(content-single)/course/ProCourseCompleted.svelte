<script lang="ts">
	...
	import type { Completed, Lesson } from '$lib/types';
	...
	const completed = docStore<Completed>(firestore, ref);
...
<!-- Lesson -->
{#if lesson}
	{#if $completed?.lesson?.filter((l) => l.id === lessonId && l.complete === true).length}
		<button
			on:click={(e) => {
				e.preventDefault();
				setDoc(
					doc(firestore, ref),
					{
						...
						complete: false,
						updated: Date.now(),
						lesson: [...($completed?.lesson?.filter((b) => b.id !== lessonId) || [])]
					},
					{ merge: true }
				);
			}}
