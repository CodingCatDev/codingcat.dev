      ...
      {posts && posts.length > 0 ? (
        <section className="relative grid gap-4 grid-cols-fit sm:gap-10">
          {posts.map((post) => {
            return (
              <div
                className="grid rounded-md shadow-lg grid-rows-auto-2 hover:shadow-2xl hover:scale-105 bg-basics-50"
                key={post._id}
              >
                      ...
                      <>
                        {post.accessSettings?.accessMode === 'closed' ? (
                          <>
                            ...
                            <p className="px-4 py-2 text-sm font-bold tracking-wider -skew-y-12 justify-self-end bg-secondary-600 dark:bg-secondary-600 text-basics-50 dark:text-basics-50">
