...
export default function SearchModal(): JSX.Element {
  ...
  const Modal = ({ handleClose, show, children }: any) => {
    const showHideclassNameName = show
      ? 'fixed inset-0 z-50 bg-basics-900 dark:bg-basics-900 dark:bg-opacity-50 bg-opacity-50 grid place-items-center h-full'
      : 'hidden';
    ...
    <>
      {show && (
        <Modal show={show} handleClose={hideModal}>
          <section
            className="grid w-full max-w-lg grid-cols-1 mx-auto transition-all rounded-md shadow-md min-h-1/2 bg-basics-50 text-basics-900"
            style={{
              animation:
                'scale-in-center 0.3s cubic-bezier(0.250, 0.460, 0.450, 0.940) both',
            }}
          >
            <section className="relative grid grid-cols-1 gap-4 p-4 grid-rows-search">
