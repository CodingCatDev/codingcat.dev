...
const systemSvg = (
  <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6 mr-2">
    <path
      ...
      strokeWidth={2}
      strokeLinejoin="round"
      className="stroke-primary-500 fill-primary-400/20"
    />
    <path
...
export default function Toggle(): JSX.Element {
  ...
  const theme = [
    { name: 'light', svg: lightSvg, text: 'Light', click: disable },
    { name: 'dark', svg: darkSvg, text: 'Dark', click: enable },
    { name: 'system', svg: systemSvg, text: 'System' },
  ];
  const [selected, setSelected] = useState(isDarkMode ? theme[1] : theme[0]);

  const setTheme = (e: any) => {
    const t = theme.find((t) => t.name === e) || theme[0];
    setSelected(t);
    if (t.name === 'light') {
      disable();
      document.documentElement.classList.remove('dark');
    }
    if (window && window.localStorage && t.name === 'system') {
  ...
  return (
    <Listbox value={selected} onChange={setTheme}>
      ...
      <Listbox.Options className="absolute right-0 z-50 py-1 overflow-hidden text-sm font-semibold bg-white rounded-lg shadow-lg top-full ring-1 ring-primary-900/10 w-36 text-primary-700 dark:bg-primary-800 dark:ring-0 dark:highlight-white/5 dark:text-primary-300">
        {theme.map((t) => (
